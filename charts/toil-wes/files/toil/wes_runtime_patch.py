...
if {{ ternary "True" "False" (not .Values.s3.realaws) }}:
    patch_file(
        '/usr/local/lib/python3.13/dist-packages/toil/jobStores/aws/jobStore.py',
        [
            ('public=True,', 'public=False,'),
            ('encryptable=True,', 'encryptable=False,'),
        ],
    )
    if {{ ternary "True" "False" (.Values.celery.enabled) }}:
        patch_file(
